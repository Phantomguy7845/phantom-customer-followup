...
const renderProducts = () => {
  ...
  if (!state.products.length) {
    empty.textContent = 'ยังไม่มีสินค้า active/promotion';
    return;
  }
  ...
  state.products.forEach((product) => {
    ...
    card.innerHTML = `
      <header>
        ...
        ${product.status === 'promotion' ? '<span class="badge">Promo</span>' : ''}
      </header>
      <p class="muted">${product.description || 'ไม่มีรายละเอียด'}</p>
      <div class="price-row">
        ${
        ...
        <strong>${formatCurrency(product.status === 'promotion' && product.promo_price
          ? product.promo_price
          : product.base_price)}</strong>
      </div>
      <div class="quantity">
        <label class="field">
          <span>จำนวน</span>
  ...
  try {
    ...
    result.innerHTML = `
      <div class="order-code">
        ...
        <button type="button" class="ghost" id="copy-order">คัดลอก</button>
      </div>
      <p class="small muted">ระบบรีเซ็ตเลขออเดอร์ทุกต้นเดือน</p>
    `;
    ...
    result.innerHTML = `
      <div class="mini-grid">
        <div>
          ...
          <p class="status-chip">${order.order_status}</p>
        </div>
        <div>
          <p class="muted">สถานะชำระ</p>
          <p class="status-chip">${order.payment_status}</p>
        </div>
        <div>
      ...
      <div class="hint">วิธีชำระ: ${order.payment_method}</div>
